...
> 制定日期：2026-08-17
>
> 当前阶段：M1 本地可安装插件已完成，下一步执行 M2 公开化改造
>
...
| 编号 | 状态 | 任务 | 修改位置 | 验收标准 |
|---|---|---|---|---|
| P10 | [ ] | 抽取内容生产公共规则 | `create-creator-content/SKILL.md` 与 `references/` | 支持对话输入和结构化输入；不要求仓库专用文件 |
| P11 | [ ] | 改造事实与案例边界 | 内容规则 reference | 缺少案例时输出问题，不生成虚构经历或指标 |
| P12 | [ ] | 按平台选择动态生成发布包 | 内容 Skill | 选择一个平台时只生成一个平台，不默认四平台 |
| P13 | [ ] | 抽取 HTML 和封面能力 | `produce-creator-visuals` | 只有已确认内容才能进入视觉生产；无生图工具时明确降级 |
| P14 | [ ] | 抽取视频场景规划能力 | `plan-creator-video` | 支持 `direct-content` 与 `accepted-html`，不承诺渲染 MP4 |
| P15 | [ ] | 删除公开版内部标记 | 三个公开 Skill | 不暴露 `CW-SKILL-1.0`、Bridge 路径或内部状态字段 |
| P16 | [ ] | 优化触发描述和默认提示 | 三个 `SKILL.md`、`agents/openai.yaml` | 直接、间接和不完整请求可触发；无关任务不触发 |
| P17 | [ ] | 建立统一输出降级说明 | 各 Skill reference | 缺浏览器、搜索、生图、文件写入时给出真实且可继续的替代结果 |
...
| 编号 | 状态 | 任务 | 交付物 / 验收标准 |
|---|---|---|---|
| P24 | [ ] | 建立 5 个正向市场测试 | `tests/plugin/positive-cases.json` | 覆盖内容、案例不足、HTML、封面、单平台发布和视频方案中的至少五类 |
| P25 | [ ] | 建立 3 个负向触发测试 | `tests/plugin/negative-cases.json` | 编程、纯视频剪辑、内容库开发等无关请求不触发 |
| P26 | [ ] | 建立边界测试 | `tests/plugin/edge-cases.json` | 覆盖无搜索、无生图、无写权限、未确认内容和输入冲突 |
| P27 | [ ] | 建立结构与隐私扫描 | CI、现有 `check:privacy` | 无密钥、个人路径、认证文件、真实私人案例和未授权素材 |
| P28 | [ ] | 新任务前向测试 | Codex 新任务测试记录 | 每个 Skill 至少进行一次无上下文泄漏的真实调用 |
| P29 | [ ] | 全仓回归 | npm scripts | lint、build、unit、bridge、plugin validate、privacy 全部通过 |
...
| 编号 | 状态 | 任务 | 交付物 / 验收标准 |
|---|---|---|---|
| P30 | [ ] | 确定正式品牌名称与开发者名称 | manifest、商店资料 | 名称清晰、非通用占位、不暗示 OpenAI 官方背书 |
| P31 | [ ] | 制作 Logo 和市场截图 | `plugins/.../assets/` | 浅色/深色可读，截图只使用通用示例数据 |
| P32 | [ ] | 编写商店短描述、长描述和起始提示词 | `plugin.json`、提交材料 | 与实际能力一致，起始提示不超过平台限制 |
| P33 | [!] | 准备官网与支持地址 | 公共 HTTPS 页面 | 用户可访问，包含使用说明和联系方式 |
| P34 | [!] | 准备隐私政策和服务条款 | 公共 HTTPS 页面 | 说明数据处理、文件访问、第三方能力和责任边界 |
| P35 | [ ] | 整理测试证据和版本说明 | `submission/` 或外部提交表单资料 | 5 正向、3 负向、可用地区、release notes 完整 |
...
下一批次只执行 M2 的 P10～P17：完善普通用户对话入口、事实边界、单平台发布、HTML/封面、视频方案、触发描述和能力降级。继续不迁移 Bridge、不删除旧 Skill、不修改当前内容工作站生产行为。
