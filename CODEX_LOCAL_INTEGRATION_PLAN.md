...
> 状态：A0～A6 已完成；个人本地版通过真实选题全链路验收，GitHub 本地分发版待启动
...
### M4：GitHub 本地分发（阶段 B，暂不执行）

| 编号 | 状态 | 任务 | 验收标准 |
|---|---|---|---|
| C25 | [ ] | 增加一键本地启动命令 | `npm run dev:local` 同时启动前端和 Bridge |
| C26 | [ ] | 编写首次使用引导 | 从安装 Codex、登录到生成第一条内容步骤完整 |
...
阶段 A 初步规划为 7 个 Codex 工作批次。若 A0 或 A1 发现关键能力不可用，先完成替代方案验证，不继续堆叠页面功能。
