...
> 状态：A0～A8、B0～B4 已完成；本地直连版本 `v0.1.0` 发布
...
| C29 | [x] | 清理隐私与本机路径 | Git 扫描不包含个人内容、认证文件、密钥和绝对路径 |
| C30 | [x] | 新机器克隆验收 | 在第二个干净目录完成 clone → install → doctor → build → start/stop；复用本机 Codex 登录，仅执行不消耗额度的运行验收 |
| C31 | [x] | 发布版本 | 构建、测试与远端 CI 通过后快进 main，并创建本地直连版本 `v0.1.0` 标签 |
...
#### B3 隐私扫描与干净目录克隆验收（2026-08-11）

- 新增 `npm run check:privacy`，扫描全部 Git 跟踪文件中的密钥、认证文件、本机数据目录和用户绝对路径；CI 在 lint、构建和 Fake Runner 测试前执行该检查。
- 自动测试增加隐私扫描负向场景，确认测试仓库出现模拟密钥时会返回失败；当前仓库 82 个跟踪文件扫描通过。
- 从 GitHub 远程分支通过 HTTPS 克隆到第二个干净目录，确认不包含 `.data`、`work`、`.env.local` 或个人账号资料。
- 干净目录执行 `npm ci`、隐私扫描、9 项 Doctor、生产构建和 `npm run dev:local` 均通过；网页返回 HTTP 200，Bridge 健康接口正常。
- 停止测试进程后确认 3000 与 4317 端口均释放。由于使用同一台电脑，Codex 登录沿用本机认证；真实额度链路已在 B2 冒烟验收，不在干净克隆中重复消耗。
- `npm ci` 的审计结果包含 21 项上游依赖风险（1 低、4 中、16 高）。B4 发布前需完成兼容性评估和回归验证，不使用 `npm audit fix --force` 直接破坏性升级。
- 阶段结论：C29、C30 完成，GitHub 本地分发基础验收通过，可以进入 B4。
...
| B4 | 发布前依赖审计与 C31：构建、提交、推送和版本标签 | 中 | 已完成 |

阶段 B 的 B0～B4 已全部完成；后续以真实本地用户反馈驱动安装体验和兼容性迭代。
